...
const categoriesContainer = document.getElementById('categories'); // Assurez-vous d'avoir cet élément dans votre HTML
...
async function fetchCategories(categoryIds) {
    try {
        ...
        categoriesContainer.innerHTML = selectedCategories.map(category => `<span>${category.name}</span>`).join(', ');
